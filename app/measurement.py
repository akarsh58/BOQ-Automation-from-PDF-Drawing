...
def _deduct_openings(wall: MeasuredElement, openings: list[MeasuredElement]) -> tuple[float, float, list[str]]:
    ...
    for opening in openings:
        ...
        if area is None:
            ...
            continue
        # IS 1200 deduction rule: deduct all openings > 0.1 sqm.
        # Do not apply an additional length threshold; a 0.6 m x 0.2 m
        # opening is 0.12 sqm and must still be deducted.
...
def _same_wall(first: MeasuredElement, second: MeasuredElement, tolerance: float = 0.05) -> bool:
    """Check if two walls represent the same physical wall, allowing for tracing imprecision.
    
    Two walls are considered the same if their endpoints are within ``tolerance``
    metres of each other (in either direction), and they have the same thickness.
    """
    first_pts = first.centerline_m
    second_pts = second.centerline_m
    if not first_pts or not second_pts or len(first_pts) < 2 or len(second_pts) < 2:
        return False
    a1, b1 = first_pts[0], first_pts[-1]
    a2, b2 = second_pts[0], second_pts[-1]
    ...
    return (
        ...
        _point_distance(a1, b2) <= tolerance and _point_distance(b1, a2) <= tolerance
    )


def _wall_key(wall: MeasuredElement) -> Optional[tuple]:
    """Return a stable key for a wall's centerline endpoints and thickness.
    
    NOTE: This key is only used for exact matches. For tolerance-based deduplication
    (e.g., manual tracing imprecision), use ``_same_wall`` in the measure_takeoff loop.
    """
...
def measure_takeoff(doc: TakeoffDocument, kb: pd.DataFrame) -> tuple[list[MeasurementLine], list[Deficiency]]:
    ...
    # Tolerance-based wall deduplication: compare each wall against
    # previously seen walls using _same_wall instead of exact key match.
    _WALL_DEDUP_TOLERANCE = 0.02  # 2 cm tracing imprecision
    seen_walls: list[MeasuredElement] = []
    for wall in walls:
        is_dup = False
        for prev in seen_walls:
            if _same_wall(wall, prev, tolerance=_WALL_DEDUP_TOLERANCE):
                is_dup = True
    ...
    for wall in walls:
        ...
        # Plaster/paint faces: shared walls are finished on both sides;
        # otherwise count one face per adjoining space.  Do not infer
